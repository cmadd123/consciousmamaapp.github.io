...
class _CategoryDetailsLocalProducWidgetState
  ...
  // Show tag editor for correcting recipe tags
  void _showTagEditor() {
    final recipe = widget.itemDetails;
    if (recipe == null) return;

    // Current values
    String selectedMealTyp = recipe.mealTyp;
    String selectedRecipeType = recipe.recipeType?.name ?? 'Entree';
    String selectedMainOrSides = recipe.mainOrSides.isEmpty ? 'Main' : recipe.mainOrSides;
    ...
    showModalBottomSheet(
      ...
      builder: (sheetContext) => StatefulBuilder(
        builder: (context, setSheetState) => Container(
          ...
          padding: EdgeInsets.fromLTRB(24.0, 16.0, 24.0, MediaQuery.of(context).padding.bottom + 24.0),
          child: Column(
            mainAxisSize: MainAxisSize.min,
            ...
            children: [
              ...
              SizedBox(height: 8.0),
              Text(
                'Correct the tags for "${recipe.recipeName}"',
              ...
              Wrap(
                ...
                children: ['Breakfast', 'Lunch', 'Dinner', 'Snacks'].map((type) {
                  final isSelected = selectedMealTyp.toLowerCase().contains(type.toLowerCase());
                  return FilterChip(
                    label: Text(type),
                    selected: isSelected,
                    onSelected: (selected) {
                      setSheetState(() {
                        if (selected) {
                          ...
                          if (selectedMealTyp.isEmpty) {
                            selectedMealTyp = type;
                          } else if (!selectedMealTyp.toLowerCase().contains(type.toLowerCase())) {
                            selectedMealTyp = '$selectedMealTyp, $type';
                          }
                        } else {
                          // Remove from list
                          final types = selectedMealTyp.split(',').map((s) => s.trim()).where((s) => s.toLowerCase() != type.toLowerCase()).toList();
                          selectedMealTyp = types.join(', ');
                        }
                      });
                    },
              ...
              Wrap(
                spacing: 8.0,
                runSpacing: 8.0,
                children: ['Entree', 'Side', 'Drink'].map((type) {
                  final isSelected = selectedRecipeType == type;
                  return ChoiceChip(
                    label: Text(type),
                    selected: isSelected,
                    onSelected: (selected) {
                      if (selected) {
                        setSheetState(() {
                          ...
                          // Also update main_or_sides for compatibility
                          selectedMainOrSides = type == 'Side' ? 'Side' : 'Main';
                        });
                      }
                    },
                    selectedColor: FlutterFlowTheme.of(context).primary.withOpacity(0.2),
                  ...
                  onPressed: () async {
                    ...
                    try {
                      ...
                      await recipe.reference.update({
                        'meal_typ': selectedMealTyp,
                        'recipe_type': selectedRecipeType,
                        'main_or_sides': selectedMainOrSides,
                      });
                      ...
                      if (mounted) {
                        scaffoldMessenger.showSnackBar(
                          SnackBar(
                            content: Text('Error updating tags'),
                            backgroundColor: Colors.red,
                            behavior: SnackBarBehavior.floating,
                            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)),
                          ),
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          // Tag Editor (for all recipes - manual tag correction)
          _buildActionButton(
